...
static func validate(ids: Array) -> Array[String]:
	...
	for ability_id in ids:
		...
		if definition.has("aura"):
			...
			if str(aura.get("event", "")) not in ["before_attack", "before_defend", "objective_control", "battle_shock"]:
			...
			if not (conditions is Dictionary):
				...
				for field in conditions:
					var allowed := ["COMMAND", "MOVEMENT", "SHOOTING", "CHARGE", "FIGHT"] if field == "phase" else ["SHOOT", "FIGHT", "OBJECTIVE_CONTROL", "BATTLE_SHOCK"]
