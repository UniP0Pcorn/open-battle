...
func run() -> void:
	...
	check(lobby_probe.host_relay("http://relay.invalid", "relay-room") == "INVALID RELAY URL", "lobby validates relay host endpoints")
	AccountStore.remove_identity(identity_path)
	...
	check(lobby_scene.directory_url != null and lobby_scene.directory_status != null and lobby_scene.room_results != null and lobby_scene.relay_url != null, "lobby exposes optional public room directory controls")
