...
曲射武器可以指定被矩形地形挡住的目标；规则层对该攻击加 1 命中阈值并给予目标掩体修正，普通武器仍需要清晰视线。
...
`rules/replay.gd` 可以从初始棋盘和命令日志重建移动、前进、冲锋、回合、战斗震慑以及射击/近战/危险武器造成的伤口状态，并拒绝序列断裂、未知单位、非法伤害事件或越权命令。
`rules/command_schema.gd` 集中定义十类命令的字段和阶段契约；存档、回放和未来服务器共享同一验证入口，避免不同入口接受不同的命令格式。
