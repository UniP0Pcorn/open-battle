...
| F7 / 战斗震慑检定 | 指挥阶段逐单位提交战斗震慑检定；联机由主机私有随机源生成并记录 2D6 |
...
当前回归：Godot **592 项检查，0 失败**；Python 数据管线 **15 项测试通过**。
...
已接入敌方移动后的能力授予反应窗口（AFTER_ENEMY_MOVE）：主机暂停普通命令，响应方可在中文面板选择策略与单位，或放弃；日志及重连快照保留窗口。仅在兵牌声明可用策略时触发。数据定义的反应射击已接入主机结算；旧 fire_overwatch 声明仍拒绝且不扣点。

临时策略能力支持本阶段（PHASE）、本回合（TURN）和整局（BATTLE）期限，过期时保留原生能力和其他有效授予。网络协议已升级至 v2，修复 JSON 坐标与数字类型导致的快照哈希不一致；联机双方需更新至相同版本。
...
联机移动结算：前进与冲锋只提交意图，由主机掷骰；失败冲锋也会记录并阻止重复尝试。冲锋仍采用现有单底座直线原型。

阵营防御光环支持豁免重掷与无敌豁免，并按当前桌面状态参与射击及近战结算。测试中的光环为原创功能夹具。

光环可按阶段和攻击种类限定生效；防御能力可通过数据策略限时授予，并随期限自动失效。

自定义策略支持目标关键词限制；目标不符时不会扣除指挥点或执行效果。

反应策略面板会随策略选择刷新合格单位、射手与武器；空候选不能执行，仍可放弃反应。

自定义策略支持数据驱动的阶段、回合和整场使用次数上限；失败请求不消耗次数。
...
仓库还提供依赖零的透明 WebSocket 中继原型：

```powershell
python -m tools.relay_server --host 0.0.0.0 --port 8766
```

它按房间配对一个 `host` 和一个 `client`，只转发不透明的 UTF-8 JSON 帧，不解析规则、不生成骰子，也不能替代主机权威。当前 Godot 大厅仍使用 ENet 直连；中继服务已完成协议和独立测试，客户端 WebSocket 传输接入仍待完成。公网部署前必须加入 TLS、认证、限流和来源策略。
