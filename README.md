...
当前回归：Godot **460 项检查，0 失败**；Python 数据管线 **7 项测试通过**。

已接入敌方移动后的能力授予反应窗口（AFTER_ENEMY_MOVE）：主机暂停普通命令，响应方可在中文面板选择策略与单位，或放弃；日志及重连快照保留窗口。仅在兵牌声明可用策略时触发。警戒射击执行器仍待实现，现会明确拒绝且不扣点。
