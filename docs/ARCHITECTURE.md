...
运输工具使用 `EMBARK`、`TRANSPORT_MOVE` 和 `DISEMBARK` 命令；模型通过 `embarked_in` 绑定运输工具，容量、3 英寸搭载/下车范围、运输移动后禁止下车和乘员同步位移都在回放层验证；附属角色组会作为同一乘员组搭载和下车。
单位关键词 `fly` 现在参与移动判定：飞行单位可以越过地形路径，但终点仍须满足桌面边界、地形落点和底座重叠约束。

附属角色使用 ATTACH/DETACH 命令记录 leader/bodyguard 关系；角色和本体以共享 group id 参与移动、前进、撤退、战斗震慑和 Fights First 状态，回放、单机 AI、桌面和联机快照共用相同字段。
