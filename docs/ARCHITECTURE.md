...
回放器已经能重建阶段切换、移动、前进、撤退、冲锋、回合、战斗震慑、伤害和危险武器事件；阶段切换使用 `PHASE_ADVANCE` 命令，跳过阶段会被拒绝。移动回放现在复用路径、底座碰撞、桌面边界和矩形地形判定，不能只靠伪造最终坐标通过权威校验。状态快照、规则集版本、服务器权威提交和网络房间生命周期已经接通。3D 客户端可重用同一英寸制规则，替换屏幕坐标映射即可。
