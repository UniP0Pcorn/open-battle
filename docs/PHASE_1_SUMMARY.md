...
643 条是从来源 PDF 自动切出的候选行，包含重复单位、页眉误识别、单模型/多模型分值混排和能力文本混入，不能直接当作 643 张可用兵牌。当前复核表的主要原因是：530 条缺少可确认的分值，270 条含尚未接入执行器的武器关键词，212 条武器伤害不是固定数值，196 条攻击次数不是固定数值，89 条没有可靠武器行，另有 24 条移动值和 18 条目标控制值需要人工确认。它们可以同时命中多个原因，所以这些数字不能相加得到 643；复核目标是把候选压缩成可追溯、可验证的正式 profile。
