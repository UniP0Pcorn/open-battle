...
- 数据索引：30 个来源记录、30 个待复核 profile、31 个目录索引条目；候选工作区当前有 643 个草稿，48 条结构字段完整候选，其中 26 条已通过当前可执行武器标签筛选，仍需人工确认底座、阵营与编成字段。

643 条是从来源 PDF 自动切出的候选行，包含重复单位、页眉误识别、单模型/多模型分值混排和能力文本混入，不能直接当作 643 张可用兵牌。当前复核表的主要原因是：530 条缺少可确认的分值，340 条含尚未接入执行器的武器关键词，239 条武器伤害不是固定数值，204 条攻击次数不是固定数值，23 条没有可靠武器行，另有 24 条移动值和 18 条目标控制值需要人工确认。近战武器行现在也会进入草稿，复合关键词会拆分处理；因此当前有 26 条可直接进入“待确认底座/阵营”的候选。它们可以同时命中多个原因，所以这些数字不能相加得到 643；复核目标是把候选压缩成可追溯、可验证的正式 profile。
