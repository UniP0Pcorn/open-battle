...
643 条是从来源 PDF 自动切出的候选行，包含重复单位、页眉误识别、单模型/多模型分值混排和能力文本混入，不能直接当作 643 张可用兵牌。当前复核表的主要原因是：89 条仍缺少可确认的分值，246 条含尚未接入执行器的武器关键词，239 条武器伤害不是固定数值，204 条攻击次数不是固定数值，23 条没有可靠武器行，另有 24 条移动值、18 条目标控制值、53 条单位关键词和 39 条阵营关键词需要人工确认。新增的分数解析会识别“单位构成 一台……250分”和“3+个 280分”等短格式，并优先采用明确的单位构成价格；“反步兵/反载具/反飞行/反巨兽”以及繁体武器关键词会映射到已有规则。关键词提取现在同时支持简体分栏、同一行关键词和繁体 10E 标签；近战武器行也会进入草稿，复合关键词会拆分处理；因此当前有 175 条可直接进入“待确认底座/阵营”的候选。它们可以同时命中多个原因，所以这些数字不能相加得到 643；复核目标是把候选压缩成可追溯、可验证的正式 profile。
