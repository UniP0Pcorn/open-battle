...
- 新增 `rules/command_schema.gd`，统一定义 MOVE、ADVANCE、FALL_BACK、DEPLOY_RESERVE、SHOOT、CHARGE、FIGHT、PHASE_ADVANCE、END_TURN、BATTLE_SHOCK、HAZARDOUS 和 STRATAGEM 十二类命令。
...
当前结果：**351 项检查，0 失败**。联机攻击意图还会由主机记录危险武器自伤及对应 FNP 骰面；重放会执行临时掩体、自动通过战斗震慑、阵营自定义策略、首发优先级和每单位每战斗阶段一次的近战激活约束，单机、主机和 AI 共用目标隐匿掩体修正，断线重连快照保持一致，预备队入场状态也会进入权威快照。

## 下一步

下一阶段先把权威会话接入 P2P 包、ENet 传输、挑战认证和断线重连，再继续从用户提供的本地来源逐条复核并晋升 profile；只有审核通过的兵牌进入可用目录。
