...
当前结果：**383 项检查，0 失败**。联机攻击意图还会由主机记录危险武器自伤及对应 FNP 骰面；重放会执行临时掩体、自动通过战斗震慑、阵营自定义策略、首发优先级和每单位每战斗阶段一次的近战激活约束，单机、主机和 AI 共用目标隐匿掩体修正，回合结束统一进入下一方指挥阶段，任务目标、控制半径、比分、胜利阈值和比赛结束状态随权威快照同步，数据表会校验阵营能力与策略声明，大厅主机可启动共享原型编成并广播权威会话，账号信任记录可持久化导入，断线重连快照保持一致，预备队、斥候、角色附属、运输工具和飞行移动状态也会进入权威命令日志。
